...
<body class="bg-[#0b1120] text-slate-200 min-h-screen antialiased overflow-x-hidden selection:bg-indigo-500/30">
    ...
    <script>
        const SUPABASE_URL = "https://ieiahjgdmzjccgtafiij.supabase.co";
        const SUPABASE_ANON_KEY = "sb_publishable_Z_5GK-yN4tJ_0XmexiQVIA_oXY8r8Wk";
        let _supabase;

        // Proteksi inisialisasi library Supabase agar aman di Mobile Chrome
        try {
            _supabase = supabase.createClient(SUPABASE_URL, SUPABASE_ANON_KEY);
        } catch (err) {
            console.error("Supabase CDN failed to initialize:", err);
        }
        ...
        authForm.addEventListener('submit', async (e) => {
            e.preventDefault();
            if (!_supabase) {
                alert("Koneksi Database bermasalah. Periksa koneksi internet Anda.");
                return;
            }
            ...
            if (isLoginMode) {
                ...
                if (error) {
                    alert("Gagal Login: " + error.message + "\n(Pastikan email terdaftar & konfirmasi email sudah aktif)");
                ...
                if (error) {
                    ...
                    alert("Pendaftaran Berhasil! Silakan cek kotak masuk/spam email Anda untuk melakukan klik link verifikasi.");
                }
                btn.innerText = originalText;
            }
        });
        ...
        async function fetchBookings() {
            ...
            if (error) {
                bookingTableBody.innerHTML = '<tr><td colspan="6" class="py-8 text-center text-rose-400 text-xs">Gagal memuat tabel database. Pastikan tabel RLS Supabase diizinkan.</td></tr>';
                return;
            }
        ...
        async function deleteBooking(id) {
            if (_supabase && confirm('Yakin ingin membatalkan dan menghapus booking ini?')) {
